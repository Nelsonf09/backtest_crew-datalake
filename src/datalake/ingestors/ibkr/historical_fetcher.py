...
def fetch_crypto_m1_range(symbol: str, start_utc: str, end_utc: str, client_cfg: IBClientConfig | None = None) -> pd.DataFrame:
    ...
    try:
        ...
        for day in _date_range_days(start, end):
            ...
            # endDateTime debe ser el inicio del día SIGUIENTE para cubrir el día actual completo
            end_dt = (day + pd.Timedelta(days=1)).to_pydatetime()
            
            bars = cli.ib.reqHistoricalData(
                contract,
                endDateTime=end_dt,
                durationStr='86400 S',  # <-- CORRECCIÓN: Usar segundos para evitar ambigüedad de '1 D'
        ...
        # Concatena todos los dataframes y asegura la grilla de M1
